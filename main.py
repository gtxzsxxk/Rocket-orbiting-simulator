    ...
    def run(self):
        while True:
            ...
            if rocket.engine_throttle > 0 and rocket.engine_on == 1:
                for i in range(int(self.number/PARTICLE_DRAW_FPS
                                   * self.throttleOffset(rocket.engine_throttle)/100)):
                    nx = random.randint(-PARTICLE_RANGE,
                                        PARTICLE_RANGE)+stdx
                    ny = random.randint(-PARTICLE_RANGE,
                                        PARTICLE_RANGE)+stdy
            ...
            for i in self.particles:
                ...
                if i.x >= 800 or i.y >= 600 or i.x < 0 or i.y < 0:
                    ...
                    continue
                i.x = i.x+i.v * \
                    math.sin(rocket.direction +
                             i.direction)*1/PARTICLE_DRAW_FPS
                i.y = i.y - i.v * \
                    math.cos(rocket.direction+i.direction) * \
                    1 / PARTICLE_DRAW_FPS  # + or -????
...
class PhyObject:
    ...
    def Calculate(self):
        ...
        if self.dist != 600000:
            self.ax = self.ax-gravity_const*earthmass_const/self.dist2*self.px/self.dist
            self.ay = self.ay-gravity_const*earthmass_const/self.dist2*self.py/self.dist
        #air drag
        calcRou(self.height)
        ...
        if v > 0 and self.height < 68000:
            ...
            acc_airdrag = 0.5*rou*(self.vx*self.vx+self.vy*self.vy)*0.2*0.008
...
class Rocket (PhyObject):
    ...
    def UserForcesCompute(self):
        ...
        if (self.fuel > 0 and self.engine_on > 0) or INFINITE_FUEL == 1:
            #compute engine'
            if INFINITE_FUEL == 0:
                self.fuel = self.fuel-self.fuel_speed*sim_timebase*self.engine_throttle/100
                self.mass = self.mass-self.fuel_speed*sim_timebase * \
                    self.fuel_perweight*self.engine_throttle/100
            angle = self.direction
            # todo: should consider computing the force's direction
            #angle_earth=self.orbitangle
            angle_earth = 0
            self.ax = self.ax+self.engine_force*self.engine_throttle / \
                100*math.sin(angle+angle_earth)/self.mass
            self.ay = self.ay+self.engine_force * \
                self.engine_throttle/100*math.cos(angle+angle_earth)/self.mass
        elif self.fuel <= 0:
            self.engine_on = 0
...
arrowimg_s = pygame.transform.scale(
    pygame.image.load(r"resources\arrow.png"), (73, 56))
...
def showRocketInfor(x, y):
    ...
    textSurfaceObj = fontobj.render(
        "速度:"+format(math.sqrt(rocket.vx*rocket.vx+rocket.vy*rocket.vy), ".1f")+" m/s", True, (255, 255, 255))
...
def showPtyInfor(p,order=0):
    ...
    screen.blit(textSurfaceObj, textSurfaceRect)
    textSurfaceObj = fontobj.render(
        "速度:"+format(p.ev, ".1f")+"m/s   地表:"+format(p.height, ".1f")+"m", True, (255, 255, 255))
...
def GameStart():
    ...
    particlesys.addParticlesEffect(PARTICLESYS_EFFECT_SPOUT, PARTICLE_NUMBER,
                                   406, 407, PARTICLE_VELOCITY, 3.14+rocket.direction)
    particlesys.start()
    ...
    soud_explosion = pygame.mixer.Sound(
        r"resources\sounds\sound_explosion_debris1.wav")
    sound_engWorking = pygame.mixer.Sound(
        r"resources\sounds\sound_rocket_mini.wav")
...
def GetTips(r):
    ...
    if r.crashed == False and tipstep == 0:
        ...
        rocket.engine_on = 1
        if orbit_Mode==True:
            showTextLeftUp("轻按Z将油门设置到100%发射火箭。火箭将有适当的倾斜角度。这是正常的。",
                           4, (200, 100, 100))
        else:
            showTextLeftUp("轻按Z将油门设置到100%发射火箭。火箭将有适当的倾斜角度。这是正常的。",
                       5, (200, 100, 100))
        ...
        if r.apPoint != None:
            if r.apPoint.height > 120000:
                ...
                r.engine_throttle = 0
        if orbit_Mode == True:
            showTextLeftUp("请保持当前状态继续上升。到达下一步（轨道远点在大气层外）时我们会自动关闭引擎。", 4)
            showTextLeftUp("你需要上升到20,000米以上 此期间不要做任何操作 正在抬升火箭 请耐心等待（大概3分钟）", 5)
        else:
            showTextLeftUp("请保持当前状态继续上升。到达下一步（轨道远点在大气层外）时我们会自动关闭引擎。", 5)
            showTextLeftUp("你需要上升到20,000米以上 此期间不要做任何操作 正在抬升火箭 请耐心等待（大概3分钟）", 6)
    elif tipstep == 2:
        if r.height > 91000:
            tipstep = 3
        elif r.height > 87000:
            sim_timebase = 0.01
        if orbit_Mode == True:
            showTextLeftUp("[无需开启引擎]最高点已经位于大气层外。1、按M进入轨道模式 2、轻按O键加快时间倍率", 4)
            showTextLeftUp("在轨道模式（轻按M进入）下按H/N进行地图放大/缩小 请耐心等待 时间加速自动恢复后不要再加速", 5)
        else:
            showTextLeftUp("[无需开启引擎]最高点已经位于大气层外。1、按M进入轨道模式 2、轻按O键加快时间倍率", 5)
            showTextLeftUp("在轨道模式（轻按M进入）下按H/N进行地图放大/缩小 请耐心等待 时间加速自动恢复后不要再加速", 6)
    elif tipstep == 3:
        ...
        if r.pePoint != None:
            if r.pePoint.height > 68000:
                ...
                r.engine_throttle = 0
        if orbit_Mode == True:
            showTextLeftUp("STEP1：1、轻按M进入火箭模式 2、按方向右键[→]将火箭旋转到右水平方向，到达角度后自动进入轨道模式。", 4)
            showTextLeftUp("STEP2:轻按Z加速。当入轨后，我们会自动关闭引擎。", 5)
        else:
            showTextLeftUp("STEP1：1、轻按M进入火箭模式 2、按方向右键[→]将火箭旋转到右水平方向，到达角度后自动进入轨道模式。", 5)
...
def GameLoop():
    global screen, launchpad_image, launchpad_rect, rocket_image_s, rocket_image, rocket_rect,\
    ...
    if orbit_Mode == False:
        ...
        if rocket.height < 59000:
            screen.fill((-100/68000*rocket.height+100, -149 /
                         68000*rocket.height+149, -237/68000*rocket.height+237))
        else:
            screen.fill((0, 0, 0))
            bgdimg_n = bgdimg_s
        ...
        if rocket.height < 600:
            ...
            ).move(-rocket.horizondisplacement, rocket.height)
        ...
        vx_str = ""
        if rocket.vy >= 0:
            vy_str = format(abs(rocket.vy), ".2f")+u" ↑"
        elif rocket.vy < 0:
            vy_str = "-"+format(abs(rocket.vx), ".2f") + u" ↓"
        if rocket.ay >= 0:
            a_str = format(math.sqrt(rocket.ax*rocket.ax +
                                     rocket.ay*rocket.ay)/9.8, ".2f")+u" ↑"
        elif rocket.ay < 0:
            a_str = "-"+format(math.sqrt(rocket.ax * rocket.ax +
                                         rocket.ay * rocket.ay)/9.8, ".2f") + u" ↓"
        if rocket.vx >= 0:
            ...
            vx_str = "-" + format(abs(rocket.vx), ".2f")
        statusStr = "Vy:"+vy_str+" m/s"+"   " + \
            "Vx:" + vx_str + " m/s" + "   " + \
            "加速度:" + a_str + " G" + "   " + \
            "高度:" + format(rocket.height, ".2f") + " m" + "   " + \
            "燃油:" + format(rocket.fuel, ".1f") + "/"+format(rocket.fuel_backup, ".1f") + "   " + \
            "帧率:" + format(fpsRate, ".1f") + "   "
        showTextLeftUp(statusStr)
        showTextLeftUp("节流阀:"+str(rocket.engine_throttle)+"%  引擎:" +
                       str(rocket.engine_on)+"  时间加速:"+str(int(sim_timebase/0.01))+"x", 1)
        showTextLeftUp("火箭参数  质量:"+format(rocket.mass,
                                          ".1f")+" kg  deltaV(v):"+format(
            rocket.si_vacuum*9.8*math.log(rocket.mass/(rocket.mass-(rocket.fuel
                                                                    * rocket.fuel_perweight)), 2.78), ".1f")+" m/s", 2)

        if rocket.height < 68000:
            showTextLeftUp("大气压:"+format(pressure/1000, ".2f")+" kPa   空气阻力:" +
                           format(rocket.air_dragforce/1000, ".2f")+"kN", 3)
        else:
        ...
        pygame.draw.aaline(screen, (255, 255, 255),
                           (10, 8), (10+zoom*600000, 8))
        pygame.draw.aaline(screen, (255, 255, 255), (10, 8), (10, 5))
        pygame.draw.aaline(screen, (255, 255, 255),
                           (10+zoom*600000, 8), (10+zoom*600000, 5))
        #rlp=244/267 #50/41
        rlp = 50 / 41
        planet_image = pygame.transform.scale(
        ...
        if rocket.orbit_track.__len__() > 1 and oct != None:
            if oct.flTyp == True:
                for i in range(1, rocket.orbit_track.__len__()):
                    ...
                    pygame.draw.aaline(screen, (152, 245, 255),
                                       (_fx, _fy), (_bx, _by))
                    i = i+rocket.orbit_track.__len__()/10000

        showTextLeftUp(format(zoom * 600000, ".1f") + "px per 600km")
        showTextLeftUp("节流阀:"+str(rocket.engine_throttle)+"%  引擎:" +
                       str(rocket.engine_on)+"  时间加速倍率:"+str(int(sim_timebase/0.01))+"x", 1)
        showTextLeftUp("deltaV(v):"+format(
            rocket.si_vacuum*9.8*math.log(rocket.mass/(rocket.mass-(rocket.fuel
                                                                    * rocket.fuel_perweight)), 2.78), ".1f")+" m/s", 2)
    ...
    if showOrbitDirection == True:
        fontobj = pygame.font.Font(
            r"resources\NotoSansHans-Regular.ttf", 11)
        ...
        arrowimg_rect = arrowimg.get_rect(
            center=arect.center).move(10, 480)
            ...
            if disableKey == False or forceKeyboardCtl==True:
                if event.key == pygame.K_SPACE:
                    ...
                    if sim_timebase < 0.30:
                        if rocket.engine_on == 1 and rocket.engine_throttle > 0 and rocket.fuel > 0:
                            return
